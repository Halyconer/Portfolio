...
- Components live in `src/components/` — `ui/` (design-system primitives),
  `editorial/` (text-heavy sections), `creative/` (photo/visual work).
  Reusable logic in `src/lib/`, hooks in `src/hooks/`, pages in `src/pages/`.
- Use relative imports (`./`, `../`) within `src/`, per `AGENTS.md`.
...
- Follow `DESIGN.md`: use the tokens, semantic classes and `src/components/ui/`
  primitives rather than inventing new patterns. The visual language is an
  editorial/newsprint style; keep it consistent across editorial and creative
  pages (shared masthead and colophon, `<Section>` rhythm, `<SectionHeading>`).
