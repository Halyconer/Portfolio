    ...
    return (
        ...
        >
            ...
            <div className="relative flex justify-between items-baseline font-mono text-[10px] tracking-[0.14em] text-muted">
                <span>{p.stack[0]}</span>
                <span>{p.url ? p.url.split('/')[0] || p.url : '—'}</span>
            </div>
        </div>
    )
}
...
function UrlLink({ p }: { p: Project }) {
    ...
    const cls =
        'ml-auto font-mono text-[11px] text-accent underline hover:text-accent-deep transition-colors'
    const style = { textUnderlineOffset: 4 }

    if (isInternal) {
        return (
            <Link to={p.url} className={cls} style={style}>
                {display} &#x2197;
            </Link>
        )
    }
