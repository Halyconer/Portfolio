const sideRail: Array<{ k: string; v: string }> = [
    {
        k: 'Working on',
        v: 'A thesis on calendar-effect anomalies & a Black–Scholes pricing engine.',
    },
    {
        k: 'Reading',
        v: '“My Life as a Quant”, FT Alphaville, Marginal Revolution.',
    },
    {
        k: 'Off-screen',
        v: 'A Connect 4 minimax, a Pi-controlled bulb, slow espresso.',
    },
]

export function About() {
    return (
        <section
            id="about"
            className="px-8 py-20 border-b border-rule-strong max-md:px-5 max-md:py-12 max-sm:px-4 max-sm:py-10"
        >
            <div className="font-mono text-[11px] tracking-[0.22em] uppercase text-muted mb-5">
                &sect; 01 &mdash; About
            </div>

            <div className="grid grid-cols-[1.4fr_1fr] gap-20 items-start max-md:grid-cols-1 max-md:gap-10">
                <p className="drop-cap m-0 font-serif font-light text-[1.65rem] leading-[1.4] text-ink max-md:text-[1.35rem] max-sm:text-[1.15rem]">
                    care about the boring infrastructure of markets — how prices
                    form, how strategies decay, how to size a position so you
                    don't blow up. The best questions in economics are
                    empirical; the best empirical work needs code that runs.
                </p>

                <div className="border-l border-rule-strong pl-8 grid grid-cols-1 gap-y-5 max-md:border-l-0 max-md:border-t max-md:pl-0 max-md:pt-8">
                    {sideRail.map((row) => (
                        <div key={row.k}>
                            <div className="font-mono text-[10px] tracking-[0.2em] uppercase text-muted mb-1">
                                {row.k}
                            </div>
                            <div className="font-sans text-[0.95rem] leading-[1.5] text-ink-soft">
                                {row.v}
                            </div>
                        </div>
                    ))}
                </div>
            </div>
        </section>
    )
}
