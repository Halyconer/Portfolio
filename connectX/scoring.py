...
def minimax(
    ...
    if is_terminal_node(board) or depth == 0:
        if is_terminal_node(board):
            ...
            return (
                None,
                (
                    score_position(board, 2)
                    if maximizing_player
                    else score_position(board, 1)
                ),
            )
    ...
    if maximizing_player:
        column = random.choice(
            valid_positions
        )  # select a random column as a placeholder
        max_eval = float("-inf")
...
def is_terminal_node(board):
    return (
        check_win(board, 1) or check_win(board, 2) or len(get_valid_moves(board)) == 0
    )
...
def score_position(board, piece):
    ...
    center_col = board[
        :, COL_COUNT // 2
    ]  # a single column from the board at the moment
    center_count = np.count_nonzero(
        center_col == piece
    )  # Count the number of (AI) pieces in the center column
    score += (
        5 * center_count
    )  # So the more pieces in the center column, the higher the score
